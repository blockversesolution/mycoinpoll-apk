...
class WalletViewModel extends ChangeNotifier with WidgetsBindingObserver {
  ...
  static const String SALE_CONTRACT_ADDRESS =
      '0x732c5dFF0db1070d214F72Fc6056CF8B48692506';
  static const String ECM_TOKEN_CONTRACT_ADDRESS =
      '0x4C324169890F42c905f3b8f740DBBe7C4E5e55C0';
  static const String STAKING_CONTRACT_ADDRESSLIVE =
      '0x878323894bE6c7E019dBA7f062e003889C812715';
  static const String AGGREGATO_RADDRESS =
  ...
  static const String SALE_CONTRACT_ADDRESS =
      '0xf19A1ca2441995BB02090F57046147f36555b0aC';
  static const String ECM_TOKEN_CONTRACT_ADDRESS =
      '0x6f9c25edc02f21e9df8050a3e67947c99b88f0b2';
  static const String STAKING_CONTRACT_ADDRESSLIVE =
      '0x6c6a6450b95d15Fbd80356EFe0b7DaE27ea00092';
  static const String AGGREGATO_RADDRESS =
...
// Getters for the new properties
  ...
  Future<void> init(BuildContext context) async {
    ...
    try {
      ...
      final hasMaterial = Localizations.of<MaterialLocalizations>(
              context, MaterialLocalizations) !=
          null;
  ...
  void _subscribeToModalEvents(SharedPreferences prefs) {
    ...
    appKitModal!.onModalConnect.subscribe((_) async {
      ...
      _lastKnowChainId =
          appKitModal!.selectedChain?.chainId ?? _getChainIdFromSession();
  ...
  Future<bool> connectWallet(BuildContext context) async {
    ...
    try {
      ...
      final connected =
          await _waitForConnection(timeout: const Duration(seconds: 2));
      if (!connected) {
  ...
  /** BALANCE, DECIMAL , MINIMUM & MAX STATKE START **/

  Future<int> getTokenDecimals({required String contractAddress}) async {
    try {
      final abiString =
          await rootBundle.loadString("assets/abi/MyContract.json");
      ...
      if (decimals < 0 || decimals > 18) {
        debugPrint(
            "Invalid decimals returned from contract: $decimals, using default of 18");
  ...
  Future<String> getMinimunStake() async {
    try {
      final abiString =
          await rootBundle.loadString("assets/abi/ECMStakingContractABI.json");
      final abiData = jsonDecode(abiString);
      final decimals =
          await getTokenDecimals(contractAddress: ECM_TOKEN_CONTRACT_ADDRESS);
      ...
      final min =
          (minimumStakeResult[0] as BigInt) / BigInt.from(10).pow(decimals);
  ...
  Future<String> getMaximumStake() async {
    try {
      ...
      final max =
          (maximumStakeResult[0] as BigInt) / BigInt.from(10).pow(decimals);
  ...
  Future<String> getBalance({String? forAddress}) async {
    ...
    if (addressToQuery == null || addressToQuery.isEmpty) {
      debugPrint(
          "No valid address provided to getBalance. User wallet: $_walletAddress, Optional param: $forAddress");
      if (forAddress == null) {
        _balance = "0";
        notifyListeners();
      }
    ...
    try {
      final abiString =
          await rootBundle.loadString("assets/abi/ECMContractABI.json");
      final abiData = jsonDecode(abiString);
      ...
      debugPrint(
          '>>> Raw token balance for $addressToQuery: $rawBalanceBigInt');

      String formattedBalance;
      if (tokenDecimals == 18) {
        formattedBalance =
            EtherAmount.fromBigInt(EtherUnit.wei, rawBalanceBigInt)
        ...
        formattedBalance =
            "$wholePart.${fractionalPart.toString().padLeft(tokenDecimals, '0')}";
  ...
  Future<String> fetchLatestETHPrice({bool forceLoad = false}) async {
    ...
    if (!forceLoad &&
        cachedPrice != null &&
        cachedRawPrice != null &&
        DateTime.now().millisecondsSinceEpoch - lastUpdated < cacheDurationMs) {
    ...
    try {
      final abiString =
          await rootBundle.loadString("assets/abi/AggregatorABI.json");
      final abiData = jsonDecode(abiString);
      const aggregatorAddress = AGGREGATO_RADDRESS;
      final contract = DeployedContract(
        ContractAbi.fromJson(jsonEncode(abiData), 'EACAggregatorProxy'),
      ...
      await prefs.setInt(
          'ethPriceLastUpdated', DateTime.now().millisecondsSinceEpoch);
  ...
  BigInt? convertECMtoWei(double ecmAmount) {
    try {
      ...
      final BigInt usdNeeded =
          (ecmAmountWei * ecmPriceUSDWei) ~/ BigInt.from(10).pow(ECM_DECIMALS);

      // ETH price in USD (scaled properly with oracle decimals)
      final BigInt ethPriceUSDWei =
          rawPrice! * BigInt.from(10).pow(ECM_DECIMALS - AGGREGATOR_DECIMALS);

      // Calculate ETH needed in Wei
      final BigInt ethNeededWei =
          (usdNeeded * BigInt.from(10).pow(ECM_DECIMALS) +
                  ethPriceUSDWei -
                  BigInt.one) ~/
              ethPriceUSDWei;
      ...
      debugPrint(
          "  ETH Needed (ETH): ${ethNeededWei / BigInt.from(10).pow(18)}");
  ...
  Future<String?> buyECMWithETH(
    ...
    try {
      ...
      final abiString =
          await rootBundle.loadString("assets/abi/SaleContractABI.json");
      final abiData = jsonDecode(abiString);
      ...
      final payload =
          await _parseTransactionLogs(receipt, saleContract, walletAddress);
      // Send Payload to API
      ...
      ToastMessage.show(
        ...
        subtitle:
            "ECM purchase completed successfully. Transaction hash: $result",
    ...
    for (final log in receipt.logs) {
      ...
      try {
        ...
        final eventSignature =
            bytesToHex(ecmPurchasedEvent.signature, include0x: true);

        if (logSignature == eventSignature) {
          final decoded =
              ecmPurchasedEvent.decodeResults(log.topics!, log.data!);
          ...
          payload['paymentAmountETH'] =
              (decoded[4] as BigInt).toDouble() / 1e18;
      ...
      try {
        final logSignature = log.topics![0];
        final referralSignature =
            bytesToHex(referralRewardEvent.signature, include0x: true);

        if (logSignature == referralSignature) {
          final decodedReferral =
              referralRewardEvent.decodeResults(log.topics!, log.data!);
          ...
          payload['referralAmount'] =
              (decodedReferral[3] as BigInt).toDouble() / 1e18;
        }
      } catch (e, stackTrace) {}
    }
  ...
  Future<void> _sendPurchaseDataToApi(Map<String, dynamic> payload) async {
    ...
    if (response.statusCode != 201) {
      throw Exception(
          'Failed to send purchase data to API: ${response.statusCode}- ${response.body}');
    } else {
  ...
  Future<String?> stakeNow(BuildContext context, double amount, int planIndex,
      {String referrerAddress =
          '0x0000000000000000000000000000000000000000'}) async {
    final chainID = getChainIdForRequests();
    ...
    try {
      ...
      final tokenAbiString =
          await rootBundle.loadString("assets/abi/MyContract.json");
      final stakingAbiString =
          await rootBundle.loadString("assets/abi/ECMStakingContractABI.json");
      print(">> ABIs loaded.");

      final ecmTokenContract = DeployedContract(
          ContractAbi.fromJson(
              jsonEncode(jsonDecode(tokenAbiString)), 'eCommerce Coin'
              // 'ECommerceCoin'
              ),
          EthereumAddress.fromHex(ECM_TOKEN_CONTRACT_ADDRESS));
      ...
      final stakingContract = DeployedContract(
          ContractAbi.fromJson(
              jsonEncode(jsonDecode(stakingAbiString)), 'eCommerce Coin'),
          EthereumAddress.fromHex(STAKING_CONTRACT_ADDRESSLIVE));
      ...
      final stakeAmount =
          (Decimal.parse(amount.toString()) * multiplier).toBigInt();
      ...
      if (approveTxResult is String) {
        ...
        throw Exception(
            "Unexpected type for approve transaction hash: ${approveTxResult.runtimeType}");
      }
      ...
      if (stakeTxResult is String) {
        ...
        throw Exception(
            "Unexpected type for stake transaction hash: ${stakeTxResult.runtimeType}");
      }
        ...
        try {
          if (log.topics != null && log.topics!.isNotEmpty) {
            ...
            if (firstTopic is Uint8List) {
              ...
              firstTopicHex =
                  bytesToHex(Uint8List.fromList(firstTopic as List<int>));
            ...
            if (firstTopicHex.toLowerCase() == stakeEventHex.toLowerCase()) {
              ...
              final decodedLog =
                  stakeEvent.decodeResults(log.topics!, log.data!);
              final payload = {
                ...
                'amount': EtherAmount.fromUnitAndValue(
                        EtherUnit.wei, decodedLog[2] as BigInt)
                    .getValueInUnit(EtherUnit.ether)
                    .toString(),
              ...
              try {
                ...
                if (response.statusCode != 200 && response.statusCode != 201) {
                  throw Exception(
                      "Failed to sync stake to backend: ${response.statusCode}");
                }
              } catch (e) {
  ...
  Future<String?> forceUnstake(int stakeId) async {
    ...
    if (!_isConnected || appKitModal?.session == null || chainID == null) {
      ToastMessage.show(
          message: "Connect wallet first.", type: MessageType.error);
      return null;
    }
    _isLoading = true;
    notifyListeners();
    ...
    try {
      ...
      await getBalance();
      ToastMessage.show(
          message: "Force unstake successful!", type: MessageType.success);
      return txHash;
    } catch (e, stackTrace) {
      await logRocketTrackBlockChainEvent(
        "TRANSACTION",
      ...
      ToastMessage.show(
        message:
            isUserRejected ? "Transaction Cancelled" : "Force Unstake Failed",
        subtitle: isUserRejected
            ? "You cancelled the transaction in your wallet."
  ...
  Future<String?> unstake(int stakeId) async {
    ...
    try {
      ...
      ToastMessage.show(
          message: "Unstake successful!", type: MessageType.success);
      return txHash;
    } catch (e, stackTrace) {
      await logRocketTrackBlockChainEvent(
        "TRANSACTION",
  ...
  Future<void> unStakeCreate(String hash, List<dynamic> decodedLog) async {
    final payload = {
      ...
      'stakeId': (decodedLog[1] as BigInt).toString(),
      'amount':
          EtherAmount.fromUnitAndValue(EtherUnit.wei, decodedLog[2] as BigInt)
              .getValueInUnit(EtherUnit.ether)
              .toString(),
      'rewardAmount':
          EtherAmount.fromUnitAndValue(EtherUnit.wei, decodedLog[3] as BigInt)
              .getValueInUnit(EtherUnit.ether)
              .toString(),
  ...
  Future<void> _resolveVestedStart(
      EthereumAddress vestingContractAddress) async {
    final start = DateTime.now();
    ...
    try {
      ...
      final vestingAbiString =
          await rootBundle.loadString("assets/abi/VestingABI.json");
      ...
      final startResult = await _web3Client!.call(
          contract: vestedContract,
          function: vestedContract.function('start'),
          params: []);
      print("   Raw start: ${startResult[0]}");

      final cliffResult = await _web3Client!.call(
          contract: vestedContract,
          function: vestedContract.function('cliff'),
          params: []);
      print("   Raw cliff: ${cliffResult[0]}");

      final durationResult = await _web3Client!.call(
          contract: vestedContract,
          function: vestedContract.function('duration'),
          params: []);
      print("   Raw duration: ${durationResult[0]}");

      //'released' function that takes no parameters
      final releasedFunction = vestedContract
          .findFunctionsByName('released')
          .firstWhere((func) => func.parameters.isEmpty);
      final releasedResult = await _web3Client!.call(
          contract: vestedContract, function: releasedFunction, params: []);
      print("   Raw released: ${releasedResult[0]}");

      //'vestedAmount' function that takes one parameter (timestamp)
      final vestedAmountFunction =
          vestedContract.findFunctionsByName('vestedAmount').firstWhere(
                (func) => func.parameters.length == 1,
              );
      final claimableResult = await _web3Client!.call(
          contract: vestedContract,
          function: vestedAmountFunction,
          params: [BigInt.from(now)]);
  ...
  Future<void> claimECM(BuildContext context) async {
    ...
    try {
      ...
      final releasedFunction = vestingContract
          .findFunctionsByName('released')
          .firstWhere((func) => func.parameters.isEmpty);
      final releasedResult = await _web3Client!.call(
          contract: vestingContract, function: releasedFunction, params: []);
      print("Raw released: ${releasedResult[0]}");
      print('claimECM: Raw releasedResult = $releasedResult');

      final releasedWei =
          releasedResult.length > 0 ? releasedResult[0] as BigInt : BigInt.zero;
      if (releasedResult.length > 1) {
        print(
            'claimECM: Warning - Multiple values returned by released(): $releasedResult');
      }

      final releasedEther = EtherAmount.fromBigInt(EtherUnit.wei, releasedWei)
          .getValueInUnit(EtherUnit.ether);
  ...
  Future<void> refreshVesting() async {
    ...
    try {
      ...
      print('refreshVesting: Raw releasedResult = $releasedResult');
      ...
      if (releasedResult.length > 1) {
        print(
            'refreshVesting: Warning - Multiple values returned by released(): $releasedResult');
      }
      final releasedEther = EtherAmount.fromBigInt(EtherUnit.wei, releasedWei)
          .getValueInUnit(EtherUnit.ether);
      final releasedAmountFormatted = releasedEther.toStringAsFixed(6);
      ...
      print(
          "Vesting refreshed successfully , Released :$releasedAmountFormatted ");
    } catch (e, stackTrace) {
      await logRocketTrackBlockChainEvent(
        "REFRESH_ICO_VESTING",
        "released",
  ...
  Future<String?> startVesting(BuildContext context) async {
    ...
    try {
      ...
      final tokenAbiString =
          await rootBundle.loadString("assets/abi/ECMContractABI.json");
      final vestingAbiString =
          await rootBundle.loadString("assets/abi/ExitingVestingABI.json");
      ...
      final balanceEther = EtherAmount.fromBigInt(EtherUnit.wei, balanceWei)
          .getValueInUnit(EtherUnit.ether);

      print(
          'Existing Vesting :: User balance: ${balanceEther.toStringAsFixed(2)} ECM');
      ...
      final approveTxHash = approveTxResult is String
          ? approveTxResult
          : bytesToHex(approveTxResult);
      final approveReceipt = await _waitForTransaction(approveTxHash);
      if (approveReceipt == null) {
        throw Exception("Approval transaction failed or timed out.");
      }
      ...
      final vestingTxHash = vestingTxResult is String
          ? vestingTxResult
          : bytesToHex(vestingTxResult);
      ...
      await getExistingVestingInformation();
      print(
          'Post-vesting state: existingVestingStatus = $_existingVestingStatus, vestInfo = $existingVestInfo');
  ...
  Future<void> getExistingVestingInformation() async {
    if (!_isConnected || _walletAddress.isEmpty) {
      print(
          'getExistingVestingInformation: Wallet not connected or address empty');
      return;
    }
    _isLoading = true;
    notifyListeners();
    ...
    try {
      ...
      do {
        ...
        final countResult = await _web3Client!.call(
          contract: vestingContract,
          function:
              vestingContract.function('getVestingSchedulesCountByBeneficiary'),
          params: [EthereumAddress.fromHex(_walletAddress)],
        );
        print('getVestingSchedulesCount raw countResult: $countResult');

        count = countResult[0] is BigInt
            ? countResult[0] as BigInt
            : BigInt.from(countResult[0]);
      ...
      if (count > BigInt.zero) {
        ...
        final vestingIdFunction = vestingContract
        ...
        final releasableFunction =
            vestingContract.function('computeReleasableAmount');
        ...
        final releasableBn =
            releasableBnResult[0] as BigInt; // Directly cast as BigInt
        final releasable = EtherAmount.fromBigInt(EtherUnit.wei, releasableBn)
            .getValueInUnit(EtherUnit.ether)
            .toDouble();
        existingVestInfo.claimable = releasable;
        print(
            'getExistingVestingInformation: Computed releasable = $releasable');
      } else {
      ...
      if (e.toString().contains('function does not exist') ||
          e.toString().contains('revert')) {
        print(
            'ABI/Function mismatch - Check ExitingVestingABI.json for getVestingSchedulesCountByBeneficiary');
      }
  ...
  Future<void> resolveExistingVestedStart(
      DeployedContract vestingContract) async {
    final startTime = DateTime.now();
    ...
    try {
      ...
      final now = DateTime.now().millisecondsSinceEpoch ~/
          1000; // Current Unix timestamp (~1757548800 on Sep 11, 2025)
      ...
      print(
          'resolveExistingVestedStart: Raw schedule result = $scheduleResult');

      final schedule = scheduleResult[0] as List;

      /// Safely cast to BigInt, handling potential byte array or numeric types
      final cliffBn = schedule[1] is BigInt
          ? schedule[1] as BigInt
          : BigInt.from(schedule[1]);
      final startBn = schedule[2] is BigInt
          ? schedule[2] as BigInt
          : BigInt.from(schedule[2]);
      final durationBn = schedule[3] is BigInt
          ? schedule[3] as BigInt
          : BigInt.from(schedule[3]);
      final totalVestingAmountBn = schedule[5] is BigInt
          ? schedule[5] as BigInt
          : BigInt.from(schedule[5]);
      final releasedBn = schedule[6] is BigInt
          ? schedule[6] as BigInt
          : BigInt.from(schedule[6]);
      ...
      final released = EtherAmount.fromBigInt(EtherUnit.wei, releasedBn)
          .getValueInUnit(EtherUnit.ether)
          .toDouble();
      final totalVestingAmount =
          EtherAmount.fromBigInt(EtherUnit.wei, totalVestingAmountBn)
              .getValueInUnit(EtherUnit.ether)
              .toDouble();
      final claimable = 0.0;
      ...
      print(
          'resolveExistingVestedStart: Setting status = $status (start $startVal vs now $now)');
      ...
      print(
          'Notifying listeners with existingVestingStatus: $_existingVestingStatus, existingVestingAddress: $_existingVestingAddress');
      notifyListeners();
    } catch (e) {
      ...
      if (e.toString().contains('function does not exist') ||
          e.toString().contains('revert')) {
        print(
            'ABI/Function mismatch - Check ExitingVestingABI.json for getLastVestingScheduleForHolder');
      }
      await logRocketTrackBlockChainEvent(
        "EXISTING_VESTING_DURATIONS",
        "resolveExistingVestedStart",
  ...
  Future<void> releaseECM(BuildContext context) async {
    ...
    try {
      ...
      print(
          'releaseECM: Contract initialized with address $EXITING_VESTING_ADDRESS');
  ...
  double get existingVestedAmount {
    if (existingVestInfo.start == 0 ||
        existingVestInfo.end == 0 ||
        balance == null) return 0.0;
  ...
  double get availableClaimableAmountForExistingUser {
    ...
    final released =
        existingVestInfo.released ?? 0.0; // Handle null with default 0.0
    final available = vested - released;
    debugPrint(
        'availableClaimableAmountForExistingUser: vested=$vested, released=$released, available=$available');
    return available > 0 ? available : 0.0;
  }
  ...
  Future<TransactionReceipt?> _waitForTransaction(String txHash) async {
      ...
      try {
        ...
        if (receipt != null) {
          ...
          if (receipt.status == true) {
            ...
            throw Exception(
                "Transaction failed. Please check the block explorer for details.");
          }
        }
      } catch (e, stackTrace) {
        await logRocketTrackBlockChainEvent(
    ...
    throw Exception(
        "Transaction timed out. Could not confirm transaction status.");
  }
